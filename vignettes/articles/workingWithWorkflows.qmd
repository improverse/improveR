...
On a very general level, in the context of {improveR}, a workflow is a distinct sequence of steps, grouped by analysis-trees, which transforms a distinct input into a distinct output. The precise definition of a workflow is the foundation of the analysis' reproducability and general replicability. 
...
[An ident can be an entitiyId, entityVersionId, a resourceId or a resource's dataframe containing an entityId", or a path.]{.aside}


*treeDepth*

In the example below, let's request the full worklfow leading to step `/improve-tutorial/Reporting/Step 1`. With an `treeDepth` input value of -1, the entire workflow is extracted. Note that the returned data frame also includes step 1 of the reporting analysis tree.
...
@fig-workflow-all shows the data manpulation graph of the improve client for the workflow leading to `Reporting/Step1.` It's a visual representation of the relations between the steps which were extracted via `getStep()` and subsequent function calls. 
...
**identifying files linked across steps**
 ...
 Note that the result presented in the chunk above includes also steps from the 'first' Analysis Tree, 'Data Order'. This is due to a direct link between Step 3 in 'Data Order' and Step 1 in 'Reporting'. If we want to check which files link the two steps, we can easily find this out via workflow's `internalLinks`.
...
The output below shows that the step `Modeling/Step 10` is still part of the analysis tree 'Modeling'.
```{r}
...
`rerunChagnedAndOutdated()` performs incremental workflow execution by only rerunning steps with changed or feature outdated files as well as their consumers. It is hence a convenient function to keep the workflow in sync, without having to rerun it entirely.

```{r}
#| code-summary: "Change step to render workflow outdated"
#| eval: true

envATReportingStep1 <- getStep("/improve-tutorial/  /Step 1")
...
#no outdated or changed files? #REVIEW
changedAndOutdatedFiles <- envATEDAStep18$workflow$changedAndOutdatedFiles()
nrow(changedAndOutdatedFiles)
changedAndOutdatedFiles$fileName
...
envATDAOStep6$lineage$load(treeDepth=1)
...
class(fullLineageWFTemplate)
```

## Modifying steps of a workflow template 

Once the template has been creaetd, it is possible to modify various attributes of the template workfow.
...
template workflow is definded.

```{r}
x <- loadResource("/improve-tutorial")
x$path
```


```{r}
fullLineageWFTemplate$workflow$df()
fullLineageWFTemplate$setWorkflowTreeRootFolder(x$path) 
fullLineageWFTemplate$setWorkflowTreeName("tryout")

#fullLineageWFTemplate$setWorkflowTreeRootFolder(fullLineageWFTemplate) #no folder created
#fullLineageWFTemplate$stepTemplates$stepDf
