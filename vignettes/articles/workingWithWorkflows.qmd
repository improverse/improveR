...
On a very general level, in the context of {improveR}, a workflow is a distinct sequence of steps, grouped by analysis-trees, which transforms a specific input into a specific output. The precise definition of a workflow is the foundation of the analysis' reproducability and general replicability. 

{ImproveR} provides furthermore the ability to take an already created worklfow and to transfer it to another repository, while keeping inter-step relations, file input and output speficiations as well as tools and their definitions in order. By this, researchers can easily apply an available analytical workflow in a different context and investigate, e.g. how outcomes differ due to a different inputs while keeping the chain of steps unaltered. Rather than re-creating each individual step from scratch,
portability ensures an efficient application of the workflow in a new context.
...
The graph above depicts improve's Data Manipulation Graph for
the entire folder. Step 62516 in the Analysis Tree 'Reporting' is its final step
in the overall workflow.


# getStep

If we are interested in obtaining the workflow leading to a specific step, the function `getStep()` is key. The function takes the ident of the step of interest as an argument and returns an environment containing all the elements describing the step. From this environment, the workflow leading to the step can be extracted via the `lineage` binding. The parameters `treeDepth` and `stepDepth` limit the upstream reach of the call, i.e. they define a cut-off point for the retrieved lineage of the step in question. Once loaded, the workflow can be accessed via the `workflow` binding. 

*treeDepth*

In the example below, let's request the full worklfow leading to step `/improve-tutorial/Reporting/Step 1`. With an `treeDepth` input value of -1, the entire workflow is extracted. Note that the returne data frame also includes step 1 of the reporting analysis tree.
...
Figure @fig-workflow-all shows the data manpulation graph of the improve client for the workflow leading to `Reporting/Step1.` It's a visual representation of the relation of the steps which were extracted via the `getStep()` and subsequent function calls. 
...
When contrasting this result with the 'full' folder as depicted in @fig-folder, it can be noticed that step `EDA/Step 18` is missing. This is because it does not form part of the workflow leading to our 'destination' step `Reporting/Step 1`.

To demonstrate this aspect, let's obtain the `usage` of `EDA/Step 18`. With a step's usage all those
steps are captured which depend further downstream in the analytical workflow on at least one file generated by the step of origin, e.g. `EDA/Step 18`. Again, `getStep()` is the relevant method.
...
The result above reveals that step `EDA/Step 18` has no usage. The `load()` function remains the sole element even after calling it. This is in line with the earlier finding, that `EDA/Step 18` is no element of the
the workflow or lineage leading to `Reporting/Step 1`.

*stepDepth*

Similar to the argument `treeDepth`, `stepDepth` also allows to set the reach of the workflow of interest.
...
## identifying files linked across steps
 ...
 Note that the result includes also steps from the 'first' Analysis Tree, 'Data Order'. This is due to a direct link between Step 3 in 'Data Order' and Step 1 in 'Reporting'. If we want to check which files link the two steps, we can easily find this out via workflow's `internalLinks`.
...
filesATDOS3ATReprotingS1 <- envATReportingStep1$workflow$internalLinks %>% 
  ...
  select(name, entityId, sourceStep, targetStep)
...
In `Reporting/Step 1` these files  show up in the `remoteFiles` binding with `asLink` set to TRUE.
 ...
 ```{r}
#| code-summary: "Get remove files of `Reporting/Step 1` and check for files"
envATReportingStep1$stepDf$remoteFiles[[1]] %>%
  as_tibble() %>%
  select(name, ident, asLink) %>%
  filter(ident %in% filesATDOS3ATReprotingS1$entityId)  
 ```

Conversly, the files are also part of the inventory of Step 3 in the AT Data Order. 
```{r}
...
inventoryATDOStep3 <- envATDOStep3$getStepInventory()$data[[1]] %>%
  ...
  filter(entityId %in% filesATDOS3ATReprotingS1$entityId)
...
# remove step
envATReportingStep1$workflow$removeStep('/improve-tutorial/Modeling/Step 10')
envATReportingStep1$workflow$removeStep('envhost1.hc.scintecodev.internal-5310:ST-59463')

```

Importantly, note that the step is not deleted. It is still present in its analysis tree.
The step was only removed from the workflow leading to `Reporting/Step 1`.

```{r}
#| code-summary: "Get workflow after removal of step"
envATReportingStep1$lineage$load(treeDepth=-1)
envATReportingStep1$workflow$df() %>%
  filter(str_detect(fullName, regex("Step 10"))) %>%
  select(fullName, sourceEntityId, rationale, description)
...
`rerunChagnedAndOutdated()` performs incremental workflow execution by only rerunning steps with changed or outdated files and their consumers. It's hence a convenient
function to keep the workflow in sync, without having to rerun it entirely.
...
#only one file identified as changed or outdated
#where do we see this in the client.
changedAndOutdatedFiles %>%
  select("fileName", "entityId", "outdatedLink", "stepEntityId")

#does the result change if entire workflow is loaded before?
envATReportingStep1$lineage$load(treeDepth=-1)

changedAndOutdatedFiles <- envATReportingStep1$workflow$changedAndOutdatedFiles() #
nrow(changedAndOutdatedFiles)

envATReportingStep1$workflow$rerunChangedAndOutdated() #BUG
```

# Workflow template

Working with workflows in {improveR} is not limited to created and reexecute workflows, but
also allows for using already existing workflows as a template which can then be adapated as needed.
...
any(is.na(workflowDf)) 

fullLineageWFTemplate <- envATReportingStep1$workflow$createTemplate() #error
...
```{r}
#try with different 
envATReportingStep1 <- getStep("improve-tutorial/EDA/Step 5")
envATReportingStep1$lineage$load(treeDepth=-1)
...
fullLineageWFTemplate$stepTemplates$...$set
fullLineageWFTemplate$setWorkflowTreeRootFolder()
