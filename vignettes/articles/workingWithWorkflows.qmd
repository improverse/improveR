...
2. Load its lineage (or usage) and define its scope using `treeDepth` and `stepDepth`. The workflow becomes available in the `workflow` binding.
...
# Load workflow
stepEnv$lineage$load(treeDepth=-1, stepDepth=-1)
...
This ensures portability across different improve repositories and installations while maintaining regulatory compliance and data lineage tracking.
...
The function systematically recreates the workflow by processing steps in dependency order, ensuring that any step's dependencies are created before the step itself. Each step is uploaded with its complete file inventory and execution history, maintaining data lineage and regulatory compliance in the target repository.
...
fullLineageWFImported <- workflowTemplateFromJSON(
  filepath = "fullLineageWF.json"
)
```
...
identical(fullLineageWFImported, fullLineageWFTemplate)
# fullLineageWFImported$realise()
library(waldo)
envDiff <- waldo::compare(fullLineageWFTemplate, fullLineageWFImported)
```
...
fullLineageWFImported$setWorkflowTreeRootFolder("/improve-tutorial/FolderForImportedWorkflow/")

fullLineageWFImported$setWorkflowTreeName("DataOrder2")
```
