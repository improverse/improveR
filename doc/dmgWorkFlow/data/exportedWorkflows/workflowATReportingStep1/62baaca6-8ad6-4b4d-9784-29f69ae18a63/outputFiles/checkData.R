...
reactable(.,
  columns = list(
    subject = colDef(name = "Subject", filterable=TRUE),
    subject_row_index = colDef(name = "Subject Row Index"),
    post_amt_index = colDef(name = "Post Amt Index")
  ),
  columnGroups = list(
    colGroup(name = "Location of missing data", columns = c("subject_row_index", "post_amt_index"))
  ),
  theme=cosmo()
) %>%
reactablefmtr::add_title("Location of missing data", font_size=18) %>% 
reactablefmtr::add_subtitle("Table presents subject nested row indices of missing conc (concentration data). The table reveals that all missing
                            data are either a subjects overall first observation, or the subject's first observation after the injection of Remifentanil (`post_amt_index)`", 
                            font_color="#7F7F7F",
                            font_size=14)
...
##Export table
htmlwidgets::saveWidget(rtConcNA, file="rtConcNA.html", selfcontained=TRUE)


# Reactable of all data
remDataStd %>%
  reactable(
    filterable = TRUE,
    defaultPageSize = 50,
    columns = list(
      id = colDef(sticky = "left"),
      subject_row_index = colDef(sticky = "left"),
      subject = colDef(sticky = "left"),
      conc = colDef(
        cell = function(value) {
          if (is.na(value)) {
            return(htmltools::tagList(
              htmltools::span(style = "color: red; font-weight: bold;", "NA")
            ))
          }
          return(value)
        }
