...
The purpose of this document is to describe `improveR`'s capacities to
handle workflows as well as other related functions.
...
To demonstrate `improveR`'s pertaining functions the folder
'improve-tutorial' under
http://envhost1.hc.scintecodev.internal:5310/repository is used.
...
The column `remoteFiles` within the dataframe returned by
...
added to the dataframe: `dependenicies` and `usage`.
...
data dataframe. After applying `detachWorflowFromResources` the
dataframe no longer contains a column with the name `entityId`. 
...
console. The changes made to the dataframe are not visibly returned.
...
('sets') it to all elements (steps) of a workflow dataframe. In other
...
to the workflow dataframe are subsequently saved to the stepCache
...
workflowElements <- workflowHandle %>% retrieveWorkflow()  #dataframe with steps of the workflow
...
#dataframe with steps of the workflow
#check treeIdent of workflow elements
workflowElements %>%
...
handleFromStep <- function(stepId) {
  ...
  step <- loadResource(stepId) #loads step; dataframe w 1 row
...
retrieveWorkflow return a dataframe with elements other than steps?
...
#if folder already exists remove it; avoid accummulating outdated Handles 
...
#BUG #@HACKLM
#when calling exportWorkflow() there are repeated prompts in the terminal to authenticate.
#Maybe as many authentication requests as there are steps in the workflow? Or folders/files
#to be exported? Currently the created folders are all empty; workflow.json however is created.

# Starting JAR: "C:\Users\rschmidt\AppData\Local\R\WIN-LI~1\4.4\improveR\IMPROV~1\lib\improve-cli-1.0.0-SNAPSHOT.jar" with parameters: clone -resource ST-54047 -repository C:/dev/git-repos/improver-base/improveR/doc/dmgWorkFlow/data/exportedWorkflows/workflowATReportingStep1/7703ef55-3662-453f-bbe4-5b9f99d96cea -userProfile improveR
# To authenticate go to: https://5371.envhost1.hc.scinteco.com:1443/realms/improve/device?user_code=XQWJ-YNRV
# Exception in thread "main" com.scinteco.improve.cli.rest.RestException: Code 500: Error requesting resource for: ST-54047
#         at com.scinteco.improve.cli.rest.RestUtil.checkResponse(RestUtil.java:17)
#         at com.scinteco.improve.cli.improveapi.v1.ResourceApiV1.getResource(ResourceApiV1.java:138)
#         at com.scinteco.improve.cli.commands.clone.CmdClone.run(CmdClone.java:75)
#         at com.scinteco.improve.cli.commands.clone.CmdClone.run(CmdClone.java:63)
#         at com.scinteco.improve.cli.ImproveCLI.run(ImproveCLI.java:45)
#         at com.scinteco.improve.cli.ImproveCLI.main(ImproveCLI.java:23)
# [1] 0
# Starting JAR: "C:\Users\rschmidt\AppData\Local\R\WIN-LI~1\4.4\improveR\IMPROV~1\lib\improve-cli-1.0.0-SNAPSHOT.jar" with parameters: clone -resource ST-57280 -repository C:/dev/git-repos/improver-base/improveR/doc/dmgWorkFlow/data/exportedWorkflows/workflowATReportingStep1/7659c6f9-68db-43ec-84a6-a122842c9dfd -userProfile improveR
# To authenticate go to: https://5371.envhost1.hc.scinteco.com:1443/realms/improve/device?user_code=DKLS-EKWA
...
In total, this folder contains `r length(folderContent)` files and directories:

-  workflow.json: a JSON file with the workflow's metadata
-  folder-pairs with handles and ending with intputFiles/outputFiles
-  unique folders with handles as their name
-  links: a folder 
...
in `workflow.json` are not stored in any particular order.
...
foldersStepHandles <- folderContent %>% str_subset(., pattern = "(Files|json|links)$", negate=T) 
foldersStepHandles
length(foldersStepHandles) #43
...
full.names = TRUE, recursive = TRUE, all.files = TRUE, include.dirs=TRUE)
folderContentStep
```


## Folder-pairs with input/output in name

```{r}
#| code-summary: "Folder-pairs with input/output files"
#| cache: true

folderContentFiles <- str_subset(folderContent, pattern = "Files$") #get folders ending with "Files"
folderContentFiles
length(folderContentFiles)
```

There are `r length(folderContentFiles)`  folders which contain input and outputfiles of `r length(folderContentFiles)/2` steps. Note that the handles in the folders names are *not* step handles.

For more details, let's look at the first folder pair.

```{r}
#| code-summary: "Check content of exported workflow folder: input/output folders"
#| cache: true

#first input/outut folder pair
handleFirst <- folderContentFiles[1] %>% str_remove("[A-Za-z]+$")
handleFirst
str_subset(folderContentFiles, pattern = handleFirst)
```
...
```{r}
#| cache: true
folderContentSteps <- str_subset(folderContent, pattern = "Files|links|json$", negate=TRUE) 
length(folderContentSteps) #CONTINUE
```
...
`executeWorkflow` takes a workflow handle or a dataframe with worfklow steps as an input and
...
dataframe which contains in each row details on a specific step -
