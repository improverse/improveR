...
There two 'handle(s)From' functions: `handleFromStep()` and `handlesFromTree()`. The first one assigns a handle to a step, the second one assigns handles to all steps in an analysis tree. Both functions create an in-memory handle which contains all metadata, processes, files, and dependencies of the step or tree. The handles are stored in the `stepCache` environment.

#TODO relation to other functions; when do I need this function
...
Note that `handlesFromTree` returns only the new handle of the tree, but not the handles of the steps. The latter are only stored in the cache environment. 
