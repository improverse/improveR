#' Get lineage (dependencies) for a step
#' 
...
getLineage <- function(step, env) {
  ...
  if (!is.null(repoVersion)) {
    ...
    if (length(versionParts) >= 2) {
      ...
      if (majorMinor >= 4.4) {
        ...
        if (!is.null(lineageResult) && lineageResult$status_code == 200) {
          return(httr::content(lineageResult))
        }
        return(list())
      }
    }
  ...
  return(getLineage_deprecated(step, env))
}

#' Get lineage for version < 4.4
#' 
...
getLineage_deprecated <- function(step, env) {
  ...
  # Filter for links
  links <- inventoryDf[inventoryDf$nodeType == "Link" | inventoryDf$nodeType == "LIV", ]
  ...
  if (nrow(links) > 0) {
    ...
    for (i in 1:nrow(links)) {
      ...
      targetId <- if (!is.null(link$targetEntityId)) link$targetEntityId else link$targetId
      if (is.null(targetId) || is.na(targetId)) next
      ...
      if (!is.null(containingStep) && 
        ...
        lineageSteps <- append(lineageSteps, list(containingStep))
