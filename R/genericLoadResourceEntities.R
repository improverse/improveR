...
#' @returns The nested data frame in `$data` - the sub-entities themselves, without the
#'   one-row wrapper the load functions put around them. Returns `NULL` when `data` carries
#'   no `data` column.
#' @export
strip <- function(data) {
  return(data$data[[1]])
