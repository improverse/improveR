...
test_that("createRelationType is idempotent (same name returns existing)|ics1694", {
  ...
  # Creating again with same name — server may return existing or error.
  # Either outcome is valid; we just verify no crash.
  second <- tryCatch(
    improveR::createRelationType(
      name = typeName,
      reverseName = paste0("reverse-", typeName),
  ...
  # If server returned the existing type, id should match
  if (!is.null(second) && !is.null(second$id)) {
    cat("Second create returned id:", second$id, "\n")
  }
})
