...
test_that("GFT1-02: import test data files into acop|ics472,ics1102", {
  ...
  stopifnot("ExampleWorkflow.zip not found in package" = !(testZip == ""))
...
test_that("GFT1-04: create and run R root step (Step 1)|ics472,ics473,ics1140", {
  ...
  stopifnot("R_RUNSERVER, R_TOOL, R_TOOL_INSTANCE env vars must be set" = hasRunServerConfig())
  ...
  tryCatch({
    ...
    stop(paste("Step 1 run failed:", e$message))
  })
...
test_that("GFT1-07: detach Step 2 from parent|ics474,ics1225,ics2046", {
  ...
  result <- tryCatch(
    ...
    error = function(e) {
      stop(paste("detachStep failed:", e$message))
    }
  )
...
test_that("GFT1-08: attach Step 1 as child of Step 2|ics474,ics1225,ics2046", {
  ...
  result <- tryCatch(
    ...
    error = function(e) {
      stop(paste("attachStep failed:", e$message))
    }
  )
...
test_that("GFT1-10: verify step inventory contains expected files|ics472,ics1213", {
  ...
  stopifnot("No child data returned" = !(is.null(children$data) || length(children$data) == 0))
...
test_that("GFT1-11: audit trail has entries for GFT folder|ics472,ics1097", {
  ...
  stopifnot("No audit data returned" = !(is.null(audit$data) || length(audit$data) == 0))
