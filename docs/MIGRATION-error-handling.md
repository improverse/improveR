...
This document describes the error handling changes introduced in this
version and what downstream code needs to adapt.

## Summary of Changes

### 1. `authenticatedREST` no longer throws exceptions

**Before:** `authenticatedREST()` had an `ignoreFail` parameter (default
`TRUE`). When `ignoreFail=FALSE` and the HTTP status was not 2xx, it
called [`stop()`](https://rdrr.io/r/base/stop.html).

**After:** `authenticatedREST()` **always returns `NULL` on failure**
and never calls [`stop()`](https://rdrr.io/r/base/stop.html). The
`ignoreFail` parameter has been removed.

**Migration:** If your code passed `ignoreFail=TRUE` or
`ignoreFail=FALSE`, remove that argument. The function signature is now:
...
**Before:** A non-NULL result from `authenticatedREST()` could still
have a non-2xx status code, requiring manual status code checks:
...
**After:** If `authenticatedREST()` returns non-NULL, it is **guaranteed
to be a 2xx response**. All non-2xx responses return NULL.
...
**New feature:** When `authenticatedREST()` returns NULL, the error
details are stored in a package-level variable accessible via
...
[`lastRestError()`](https://improverse.github.io/improveR/reference/lastRestError.md)
returns `NULL` if the last REST call succeeded. Since improveR is
single-threaded, this is safe to use immediately after any REST call.

**Migration:** If your code needs to distinguish failure types (e.g. 401
vs 404), use
[`lastRestError()`](https://improverse.github.io/improveR/reference/lastRestError.md)
instead of checking `result$status_code`:
...
**Before:** Mixed usage of direct
[`logging::logdebug()`](https://rdrr.io/pkg/logging/man/logging-entrypoints.html),
[`logging::loginfo()`](https://rdrr.io/pkg/logging/man/logging-entrypoints.html),
[`logging::logwarn()`](https://rdrr.io/pkg/logging/man/logging-entrypoints.html),
[`logging::logerror()`](https://rdrr.io/pkg/logging/man/logging-entrypoints.html)
...
**After:** All code uses the `log_*()` wrappers exclusively. The
wrappers support `redirectLogs()` for structured test output.
...
**Before:** Some functions returned `NULL` or `FALSE` silently on
failure.

**After:** Every early return in exported functions logs a
[`log_warn()`](https://improverse.github.io/improveR/reference/log_warn.md)
message explaining why the operation failed.

**Migration:** If your code suppresses warnings
(e.g. [`suppressWarnings()`](https://rdrr.io/r/base/warning.html)), be
aware that there may be more log output from failed operations. This is
informational and does not change the return values.

### 6. Redundant `tryCatch` blocks removed

**Before:** Many functions wrapped `authenticatedREST()` or
...
**After:** These redundant `tryCatch` blocks have been removed
because: - `authenticatedREST()` never throws (always returns NULL on
failure) -
[`loadResource()`](https://improverse.github.io/improveR/reference/loadResource.md)
handles its own errors internally

**Migration:** If your code wraps improveR calls in `tryCatch`: - This
still works (no breaking change) - But the error handler will never fire
for REST failures since they return NULL instead of throwing - Consider
simplifying to NULL checks instead
...
| [`lastRestError()`](https://improverse.github.io/improveR/reference/lastRestError.md) added | New exported function                                                 | No action (additive)                                        |
...
Only the first item (`ignoreFail` removal) is a **hard breaking change**
that requires code modification. All other changes are
backward-compatible.
