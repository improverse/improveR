...
``` r
unloadReviewEntries(ident, from = pwd())
```
...
No meaningful value - called for its side effect of dropping the entries
of the review from the cache, so that the next load reads the server.
